function [position,positionaux,messages] = wavedet_3D(sigdir,headir,matdir,ecgnr,ft,anot,lead,t,flags,anot_fmt,aname,dirann,messages,bufferedSignal,bufferedHeaSig)
...
% h_wb = waitbar(0,'Please wait...','Name','BioSigBrowser Info...');
t_itera = 0;
itr = 0;
N_itera = ceil((t(2)-t(1))/nsamp);
% endsamp = t(1)-1;
...
while ((endsamp+1) < t(2))
    ...
    tstart = tic;    
    itr = itr +1;
    t_estim = t_itera*(N_itera-itr);
    str_t = timestr(t_estim*1000);
%     if ishandle(h_wb)
%         waitbar(itr/N_itera,h_wb,['Process: ' num2str(itr*100/N_itera,'%0.1f') ' %, Estimated time remaining: ' str_t(1:end-4)])
%     else
%         h_wb = waitbar(itr/N_itera,['Process: ' num2str(itr*100/N_itera,'%0.1f') ' %, Estimated time remaining: ' str_t(1:end-4)],...
%             'Name','Wavedet Info...');
%     end
    %%%%%%%%%%%%%%%%%%%%%
    ...
    if isnan(ft)||ft==0||ft==1||ft==2||ft==4||ft>4
        if ft==0||ft==4 || ft==1 %MIT , Lund, Mortara %21MAR2011
            [sig,messages1,heasig] = readsignalv1v2(sigdir,ecgnr,[inisamp endsamp],ft,heasigreading,leadreading,messages); % read leadreading
            if messages1.status==0
                messages.errors=[messages.errors messages1.errors];
                warning(char(messages.errors(end)))
        ...
        elseif leadsynth_flag==3||leadsynth_flag==4||leadsynth_flag==5 % PC based delineation
            ...
            elseif leadsynth_flag==3 % considering 8 uncorrelated leads
                if ~isnan(auxleads(1:7)) & (~isnan(auxleads(8))||~isnan(auxleads(9))) %#ok<AND2>
                    ...
                    warning(char(position.warnings(end)));
...
%                     me.message = 'Error including warning information';
                    messages.warnings=[messages.warnings {'Error including warning information'}];
                end
    ...
    if  messages.setup.wavedet.filter_bank_design==0% RUTE 24NOV2011
        ...
        if endsamp < size(sig,1)
            endsamp = size(sig,1);
        end
%         samp = (inisamp + l5 -1):endsamp-d5; % Test of consistency
%         21/04/2014 Juan
    ...
    elseif messages.setup.wavedet.sig_quality==1 %external
        if ~iscell(aname_sig_quality)
            aname_sig_quality_cell{1}=aname_sig_quality(1,:); %RUTE 12ABRIL2013
        end
        anots = readannot_mat(dirann_sig_quality,aname_sig_quality_cell{1},'no_quality_segments');
        data = [anots.no_quality_segments.pos anots.no_quality_segments.pos+anots.no_quality_segments.info{1}];% timing and duration? units?
...
try
    ...
    if (qrs_flag == 2)
        aux=find(annstruct.anntyp~='N');
        annstruct.anntyp(aux)=[];
        annstruct.time(aux)=[];
        annstruct.num(aux)=[];
        annstruct.subtyp(aux)=[];
        annstruct.chan(aux)=[];
        annstruct.aux(aux,:)=[];
    end
