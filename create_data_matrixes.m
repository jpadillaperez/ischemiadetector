...
for h = 1:83
    load(['Delta_indexes_and_ICS\ics' lista(h,:)]);
    load(['Delta_indexes_and_ICS\delta_indexes' lista(h,:)]);
    for g = 1:9
        if (g<7)
...
xlswrite('data_for_spss_ics.xlsx', prepared_data_spss_ics);
xlswrite('data_for_spss_delta.xlsx',prepared_data_spss_delta);
